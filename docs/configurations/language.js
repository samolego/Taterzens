...
async function checkCache(version) {
    let cache = JSON.parse(localStorage.getItem("LanguageCache"));
    let CACHED_VERSION = JSON.parse(localStorage.getItem("TaterzensVersion"));
    if(cache == null || CACHED_VERSION != version) {
        fetchNewestLanguage(version);
    } else {
        LANGUAGE.innerHTML = cache;
    }
}
async function fetchNewestLanguage(version) {
        ...
        if(xhttp.responseText != "" && !processed) {
            ...
            while ((j = configText.indexOf("\n", i)) !== -1) {
                ...
                if(line != "" && !line.includes("*") && !line.includes("@")) {
                    ...
                    if(line.includes("public static class ")) {
            ...
            localStorage.setItem("LanguageCache", JSON.stringify(LANGUAGE.innerHTML));
            localStorage.setItem("TaterzensVersion", JSON.stringify(version));
        }
    };

    xhttp.open("GET", `https://raw.githubusercontent.com/samolego/Taterzens/${version}/common/src/main/java/org/samo_lego/taterzens/storage/TaterLang.java`, true);
    xhttp.send();
}
